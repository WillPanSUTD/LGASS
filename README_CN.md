...
**关于复现：** 上述数字为论文表 3 已发表的取值。本代码仓库**暂未附带**与该数字一一对应的本地复现 checkpoint —— 原 checkpoint 已无法获取，300 epoch 完整重跑仍在进行中。如需自行复现，请运行 `bash scripts/reproduce_paper.sh`（默认使用 `configs/paper.yaml`），由于 `pointops` 内核不确定性及 CUDA / PyTorch 版本差异，主指标可能产生约 ±0.5% 的偏差；按表 2 的训练表预算，请留出数日的 GPU 时间。
